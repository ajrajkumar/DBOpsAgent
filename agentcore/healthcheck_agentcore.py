...
# TOOL 6: Send SNS email notification
from datetime import datetime
from typing import Dict, Any
...
def send_email_notification(subject: str, message: str, severity: str = "INFO") -> Dict[str, Any]:
    """
    Send an email notification via SNS with verbose logging.
    Automatically discovers SNS topic 'agentcore-database-alerts' without needing AWS_ACCOUNT_ID.
    """
    try:
        region = os.getenv('AWS_REGION', 'us-west-2')
        topic_name = 'agentcore-database-alerts'

        print("=== SNS Notification Debug Info ===")
        print(f"AWS_REGION: {region}")
        print(f"SNS_TOPIC_NAME: {topic_name}")

        sns_client = boto3.client('sns', region_name=region)

        # Find the topic ARN by matching topic name
        print("🔍 Looking for SNS topic ARN...")
        ...
        if not topic_arn:
            print(f"❌ SNS topic '{topic_name}' not found in region {region}.")
            return {
                "status": "error",
                "error": f"SNS topic '{topic_name}' not found in region {region}."
            }
...
"""
        ...
        # Truncate and sanitize subject (SNS limit is 100 chars)
        sns_subject = f"[{severity}] Database Alert: {subject}"[:100]
...
- get_index_bloat: Detect index with excessive dead space
...
Keep responses clear and focused on practical database optimization. AVOID DUPLICATION IN RESPONSES

Only call send_email_notification ONCE after getting all the recommendations together. """
