...
if selected_page == "Databases":
    ...
    st.markdown('<div class="main-header">🚨 Database Alerts</div>', unsafe_allow_html=True)
    
    # Display currently selected database ID if any
    if 'selected_alert_database_id' in st.session_state:
        st.info(f"🎯 **Currently Selected Database ID:** {st.session_state.selected_alert_database_id}")
    ...
    if agent_alerts is not None:
        ...
        if not df.empty:
            ...
            if not selected_rows.empty:
                ...
                st.success(f"✅ Selected Alert - Database ID: **{selected_database_id}**")
                
                # Analyze alert inline on the same page
                col1, col2 = st.columns([1, 4])
                with col1:
                    if st.button("🔍 Analyze This Alert", key="analyze_selected_alert", help="Analyze the selected alert", type="primary"):
                        ...
                        with st.spinner(f"Analyzing alert: {selected_alert['Alert Name']}..."):
                            try:
                                ...
                                if analysis_result:
                                    ...
                                    st.error("❌ Failed to analyze alert")
                                    
                            except Exception as e:
                                st.error(f"❌ Analysis failed: {str(e)}")
                
                # Show additional details of selected alert
                with st.expander("📋 Selected Alert Details", expanded=True):
                    col1, col2 = st.columns(2)
                    with col1:
        ...
        st.error("❌ Failed to load alerts from database agent")
