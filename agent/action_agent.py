...
def validate_sql_security(sql: str, operation_type: str) -> dict:
    """
    Validate SQL for security risks using mutable_sql_detector patterns.
    ...
    # Check for unexpected mutating keywords
    mutating_keywords = detect_mutating_keywords(sql)
    
    # Define allowed keywords per operation type
    allowed_keywords = {
        'index': {'CREATE'},
        'analyze': {'ANALYZE'},
    ...
    # Add injection risk issues
    issues.extend(injection_risks)
    
    # Check for unauthorized mutating operations
    unexpected_keywords = set(mutating_keywords) - allowed_keywords.get(operation_type, set())
    if unexpected_keywords:
        issues.append({
            'type': 'unauthorized_mutation',
            'message': f'Unauthorized mutating keywords detected: {", ".join(unexpected_keywords)}',
            'severity': 'high'
        })
    ...
    return {
        'safe': len(issues) == 0,
        'issues': issues,
        'mutating_keywords': mutating_keywords
    }
...
def create_index_concurrently(table_name: str, column_names: str, 
    ...
    try:
        ...
        # < TODO : ADD LINE FOR CREATE INDEX >
        
        # Security validation
        security_check = validate_sql_security(create_sql, 'index')
        if not security_check['safe']:
            error_msg = "Security validation failed: " + "; ".join([issue['message'] for issue in security_check['issues']])
            logger.error(error_msg)
            return {"error": error_msg}
        
        logger.info(f"Security validation passed. Executing: {create_sql}")
...
def analyze_table(table_name: Optional[str] = None) -> str:
    ...
    try:
        ...
        # Security validation
        security_check = validate_sql_security(analyze_sql, 'analyze')
        if not security_check['safe']:
            error_msg = "Security validation failed: " + "; ".join([issue['message'] for issue in security_check['issues']])
            logger.error(error_msg)
            return f"❌ Error: {error_msg}"
        
        logger.info(f"Security validation passed. Analyzing {target}: {analyze_sql}")
        start_time = datetime.now()
...
def vacuum_table(table_name: str, analyze_after: bool = True) -> str:
    ...
    try:
        ...
        # Security validation
        security_check = validate_sql_security(vacuum_sql, 'vacuum')
        if not security_check['safe']:
            error_msg = "Security validation failed: " + "; ".join([issue['message'] for issue in security_check['issues']])
            logger.error(error_msg)
            return f"❌ Error: {error_msg}"
        
        logger.info(f"Security validation passed. Running {operation} on table '{table_name}': {vacuum_sql}")
        start_time = datetime.now()
...
🔒 SECURITY VALIDATION: All SQL operations are automatically validated for:
- SQL injection patterns (comment injection, tautologies, UNION attacks, etc.)
- Unauthorized mutating operations beyond allowed scope
- Suspicious patterns that could indicate malicious intent
...
All operations undergo security validation before execution using AWS Labs MCP security patterns."""
...
if __name__ == "__main__":
    ...
    print("\nExample actions:")
    print("- Create an index on the projects table for the proj_name column")
    print("- Analyze the projects table to update statistics")
    print("- Vacuum the employees table to reclaim space")
